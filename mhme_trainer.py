...
from evaluation import prfg, prfg_

class mh_me(chainer.Chain):
    def __init__(self, hid, n1_classes, n2_classes, n3_classes, n4_classes, n_classes):
        ...
        with self.init_scope():
            ...
            self.o5 = Linear(hid, n_classes)

            self.o = Linear(hid, n_classes)

    def __call__(self, x, x1, x2, x3, x4, x5, t):
        ...
        h5 = F.sigmoid(self.o(h5))
        loss5 = F.sigmoid_cross_entropy(h5, t) 
...
class Trainer_all(object):
    ...
    def __init__(self, epoch, data, data1, data2, data3, data4, data5, label, model, optimizer, batchsize):
    ...
    def run(self, test_data=None, test_data1=None, test_data2=None, test_data3=None, test_data4=None, test_data5=None, test_label=None):      
        hid1 = []
        hid2 = []
        for epoch in range(self.epoch):
            ...
            for i in range(0, self.data_length , self.batchsize):
                ...
                _, _, _, _, _, _, loss1, loss2, loss3, loss4, loss5, loss6 = self.model(x_batch, x_batch1, x_batch2, x_batch3, x_batch4, x_batch5, y_batch)
            ...
            if test_data is not None:
                ...
                for i in range(0, test_data.shape[0], self.batchsize):
                    ...
                    _, _, _, _, _, _, loss_t1, loss_t2, loss_t3, loss_t4, loss_t5, loss_t6 = self.model(batch_x, batch_x1, batch_x2, batch_x3, batch_x4, batch_x5, batch_y)
        ...
        for i in range(0, self.data_length , self.batchsize):
            ...
            x_batch5 = c_gpu(self.data5[indexes1[i : i + self.batchsize]])
            y_batch = c_gpu(self.label[indexes1[i : i + self.batchsize]])
            _, _, _, _, _, h6, _, _, _, _, _, _ = self.model(x_batch, x_batch1, x_batch2, x_batch3, x_batch4, x_batch5, y_batch)
            hid1.append(h6.data)
        hid1 = np.array(hid1).reshape(self.data_length, self.n_classes)
        p, r, f, g = prfg(hid1, self.label)
        print ('train')
        ...
        for i in range(0, test_data.shape[0], self.batchsize):
            ...
            batch_x5 = c_gpu(test_data5[indexes2[i : i + self.batchsize]])
            batch_y = c_gpu(test_label[indexes2[i : i + self.batchsize]])
            _, _, _, _, _, h6, _, _, _, _, _, _ = self.model(batch_x, batch_x1, batch_x2, batch_x3, batch_x4, batch_x5, batch_y)
            hid2.append(h6.data)                
        hid2 = np.array(hid2).reshape(test_data.shape[0], self.n_classes)
        p, r, f, g = prfg(hid2, test_label) 
        print ('test')
        print ('precision:' + str(p))
        print ('recall:' + str(r))
        print ('f1-score:' + str(f))
...
class Trainer_all(object):
    ...
    def run(self, test_data, test_data1=None, test_data2=None, test_data3=None, test_data4=None, test_data5=None, test_label=None):      
        for epoch in range(self.epoch):
            start = time.time()
            print ('epoch', epoch)
            hid1 = []
            ...
            for i in range(0, self.data_length , self.batchsize):
                ...
                x_batch3 = c_gpu(self.data3[indexes[i : i + self.batchsize]])
                x_batch4 = c_gpu(self.data4[indexes[i : i + self.batchsize]])
                x_batch5 = c_gpu(self.data5[indexes[i : i + self.batchsize]])
                y_batch = c_gpu(self.label[indexes[i : i + self.batchsize]])
                self.model.cleargrads()
                h1, h2, h3, h4, h5, h6, loss1, loss2, loss3, loss4, loss5, loss_g = self.model(x_batch, x_batch1, x_batch2, x_batch3, x_batch4, x_batch5, y_batch)
                loss = loss1 + loss2 + loss3 + loss4 + loss5 + loss_g
                loss.backward()
                self.optimizer.update()
                hid1.append(h6.data)
            print (str(epoch) + ':' + str(time.time()-start))
            ...
            if test_data1 is not None:
                ...
                for i in range(0, test_data.shape[0], self.batchsize):
                    ...
                    batch_x3 = c_gpu(test_data3[indexes[i : i + self.batchsize]])
                    batch_x4 = c_gpu(test_data4[indexes[i : i + self.batchsize]])
                    batch_x5 = c_gpu(test_data5[indexes[i : i + self.batchsize]])
                    batch_y = c_gpu(test_label[indexes[i : i + self.batchsize]])
                    h1, h2, h3, h4, h5, h6, loss_t1, loss_t2, loss_t3, loss_t4, loss_t5, loss_t_g = self.model(batch_x, batch_x1, batch_x2, batch_x3, batch_x4, batch_x5, batch_y)
                    loss_t = loss_t1 + loss_t2 + loss_t3 + loss_t4 + loss_t5 + loss_t_g
                    hid2.append(h6.data)
                    final_loss += loss_t.data 
                print ('loss_test:' + str(final_loss/float(self.batchsize)))
                hid2 = np.array(hid2).reshape(test_data.shape[0], self.n_classes)
                p, r, f, g = prfg_(hid2, test_label)
                print ('f1-score:' + str(f))
                print ('precision:' + str(p))
                print ('recall:' + str(r))
                print ('G-mean:' + str(g))
