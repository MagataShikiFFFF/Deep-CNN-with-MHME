...
from hid_ae import Autoencoder
...
class Trainer_one(object):
    """docstring for Trainer"""
    def __init__(self, epoch, data, label, n_classes, model, optimizer, batchsize):
        super(Trainer_one, self).__init__()
        self.batchsize = batchsize
        self.epoch = epoch
        self.data = data
    ...
    def run(self, test_data=None, test_label=None):      
        for epoch in range(self.epoch):
            ...
            for i in range(0, self.data_length , self.batchsize):
                ...
                self.model.cleargrads()
                h1, h2, h3, h4, h5, h6, loss1, loss2, loss3, loss4, loss5, loss_g = self.model(x_batch, y_batch)
            ...
            if test_data is not None:
                ...
                for i in range(0, test_data.shape[0], self.batchsize):
                    ...
                    h1, h2, h3, h4, h5, h6, loss_t1, loss_t2, loss_t3, loss_t4, loss_t5, loss_t_g = self.model(batch_x, batch_y)
